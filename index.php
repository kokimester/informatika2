...
$query = "SELECT sub3.id, sub3.nev,sub3.discordid, sub3.elo, sum(sub3.win) as 'win', sum(sub3.lose) as 'lose' from (
    ...
    group by sub3.id
    order by sub3.elo desc, sub3.nev asc;";
}
else{
$searchBy = mysqli_real_escape_string($link,$_POST['searchBy']);

$query = "SELECT sub3.id, sub3.nev,sub3.discordid,sub3.steamid, sub3.elo, sum(sub3.win) as 'win', sum(sub3.lose) as 'lose' from (
    (select mind.id, mind.nev,mind.discordid,steamid, mind.elo, 0 as 'win', 0 as 'lose' from user as mind) UNION
    (select sub1.id, sub1.nev,sub1.discordid,steamid, sub1.elo, sum(sub1.win) as 'win', 0 as 'lose' from (
    select user.id, nev,discordid,steamid, elo, count(user.id) as 'win'
    from user 
    left outer join merkozes as mk on mk.player_1_id = user.id
    where mk.player_1_points > mk.player_2_points  and mk.player_1_confirmed = '1' and mk.player_2_confirmed = '1'
    group by user.id
    ...
    group by sub1.id)
    UNION
    (select sub2.id, sub2.nev,sub2.discordid,sub2.steamid, sub2.elo,0 as 'win', sum(sub2.lose) as 'lose' from (
    select user.id, nev,discordid,steamid, elo, count(user.id) as 'lose'
    from user 
    left outer join merkozes as mk on mk.player_1_id = user.id
    where mk.player_1_points < mk.player_2_points  and mk.player_1_confirmed = '1' and mk.player_2_confirmed = '1'
    group by user.id
    ...
    where (sub3.discordid like '%$searchBy%') OR (sub3.nev like '%$searchBy%') OR (sub3.steamid like '%$searchBy%')
    group by sub3.id
    order by sub3.elo desc, sub3.nev asc;";
}
